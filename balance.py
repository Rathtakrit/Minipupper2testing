...
def add_movement(desired_roll_angle, desired_pitch_angle):
    ...
    """
    Move.balance(desired_roll_angle, desired_pitch_angle, 0.015, 0)
...
def main(use_imu=False):
    ...
    original_roll = original[0]
    original_pitch = original[1]
    ...
    while True:
        ...
        # Read imu data - Can't be used for now
        quat_orientation = (
        ...
        roll_angle = angle_converter_roll(imu_data[0], imu_data[1], imu_data[2])
        pitch_angle = angle_converter_pitch(imu_data[0], imu_data[1], imu_data[2])
        ...
        if (abs(filtered_roll_angle) > 1 or abs(filtered_pitch_angle) > 1):
